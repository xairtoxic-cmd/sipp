...
import { REVIEWS, USERS, cafeById } from "@/lib/seed";
import { useStore } from "@/lib/store";
import { Icon } from "./Icons";
import { Avatar } from "./UI";

export default function CommentSheet() {
  const { commentReviewId, setCommentReviewId, comments, addComment, me } = useStore();
              ...
              {list.map((c, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  ...
                  <div className="rounded-2xl rounded-tl-md border border-line bg-ivory px-3.5 py-2.5">
                    <p className="text-xs font-medium text-espresso">{me.name}</p>
